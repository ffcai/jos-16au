...
net_timer(uint32_t arg)
{
	...
	for (;;) {
		uint32_t cur = uptime() / NANOSECONDS_PER_MILLISECOND;
...
serve_thread(uint32_t a) {
	...
	case NSREQ_SOCKET:
		...
		break;
	case NSREQ_GETHOSTBYNAME:
	{
		struct ip_addr addr;

		r = netconn_gethostbyname(req->gethostbyname.req_name, &addr);
		if (r == ERR_OK)
			memcpy(req, &addr, sizeof(addr));
		break;
	}
	case NSREQ_INPUT:
