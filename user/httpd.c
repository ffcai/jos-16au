...
static void
log(struct http_request *req, int code)
{
	...
	if (!debug)
		return;
	now = uptime() / NANOSECONDS_PER_SECOND;
	hh = (now / 3600) % 60;
	mm = (now / 60) % 60;
	ss = now % 60;
	cprintf("%s - - [%02u:%02u:%02u] \"GET %s %s\" %d -\n",
		inet_ntoa(req->client->sin_addr), hh, mm, ss,
		req->url, req->version, code);
}
...
static void
handle_client(int sock, struct sockaddr_in *client)
{
